...
Uses the prebuilt PLUMED kernel auto-detected by `qcb.config.PLUMED_KERNEL`
(or set the env var manually).

What this enables that pure-Python `qcb.mlff.metadynamics` does not:
...
- Standard PLUMED file formats (HILLS, COLVAR) that `qcb.analysis.fes` reads
...
log = logging.getLogger("qcb.mlff.plumed_runner")
...
def write_plumed_input(spec: PlumedRunSpec, out_path: str | Path) -> Path:
    ...
    lines = ["# Generated by qcb.mlff.plumed_runner"]
...
def _setup_plumed_env() -> tuple[str, dict]:
    ...
    """
    try:
        from qcb.config import PLUMED_KERNEL
    except Exception:
        PLUMED_KERNEL = None
