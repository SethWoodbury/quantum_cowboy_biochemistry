...
log = logging.getLogger("qcb.auto_spring_k")
...
def _resolve_xtb_binary() -> Optional[str]:
    ...
    try:
        from qcb.mlff.xtb_refine import XTB_BIN as _configured
        if _configured and os.path.isfile(_configured):
            return _configured
    except Exception:
        pass
    try:
        from qcb.config import XTB_BIN as _configured  # type: ignore
        if _configured and os.path.isfile(_configured):
            return _configured
    except Exception:
        pass
