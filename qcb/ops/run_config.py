...
log = logging.getLogger("qcb.ops.run_config")
...
def _resolve_atom_indices(
    ...
    from qcb.io.constraints import parse_constraints
...
def run(config_path: str | Path, override: dict | None = None) -> dict:
    ...
    """
    from qcb.io.config import load_config
    from qcb.io import load_structure, parse_constraints, build_fix_atoms
    from qcb.calc import make_calc, make_calc_fn
    ...
    if op.kind == "sp":
        ...
        if op.backend == "plumed":
            from qcb.mlff.plumed_runner import run_bond_difference_mtd
        ...
        from qcb.mlff.plumed_runner import run_umbrella_window
        atoms.calc = make_calc(cs.model, head=cs.head, device=cs.device,
                               default_dtype=cs.dtype, charge=charge)
        idx = _resolve_geometry_atoms(op.cv, cfg.geometry, cfg.selectors, atoms, bt_struct)
