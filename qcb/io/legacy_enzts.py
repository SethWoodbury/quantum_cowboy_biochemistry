"""Translator for legacy enz-ts YAML configs to the modern qcb config format.
...
some expressions can't be cleanly mapped to the qcb selector grammar. Anything
...
Usage:
    ...
    from qcb.io.legacy_enzts import translate_legacy_yaml, translate_directory
...
# ---------------------------------------------------------------------------
# Biotite expression -> qcb selector grammar
# ---------------------------------------------------------------------------

# Patterns we recognise. The qcb selector grammar (per the spec) uses tokens
...
def translate_biotite_expr(expr: str) -> tuple[str, str | None]:
    """Translate a biotite annotation expression to a qcb selector string.
...
def translate_legacy_yaml(path: str | Path) -> dict:
    """Read a legacy enz-ts YAML config, return a dict matching the new qcb schema.
...
def translate_directory(in_dir: str | Path, out_dir: str | Path) -> list[Path]:
    """Translate every *.yaml in in_dir; write *.qcb.yaml under out_dir.
    ...
    for src in sorted(in_dir.glob("*.yaml")):
        ...
        header = [
            f"qcb config translated from legacy enz-ts: {src}",
            "Generated by qcb.io.legacy_enzts.translate_directory",
        ]
        if warnings:
            header.append(f"{len(warnings)} TODO(s) — see _translation_warnings")
        out_name = src.stem + ".qcb.yaml"
...
def _main(argv: list[str]) -> int:
    if len(argv) == 1:
        ...
        print(
            "usage: python -m qcb.io.legacy_enzts [IN_DIR OUT_DIR]\n"
