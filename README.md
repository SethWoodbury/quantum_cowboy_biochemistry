...
> The importable package stays **`quantum_engine`** and the CLI stays **`cowboy-qc`**.

## The TS pipeline: `cowboy-qc ts-entry`
...
cowboy-qc reaction-spec spec.yaml --structure site.pdb   # validate/resolve a ReactionSpec
cowboy-qc monitor site.pdb --metals --bond 0,1           # non-constraining bond/metal report
```
...
```bash
cowboy-qc sp       input.pdb                             # single-point energy
cowboy-qc opt      input.pdb --fmax 0.01                 # energy minimization
cowboy-qc md       input.pdb --time 10 --temp 300        # molecular dynamics
cowboy-qc freq     input.pdb                             # vibrational frequencies
cowboy-qc scan     input.pdb --coord bond --indices 5 12 --start 1.5 --end 3.5 --n-steps 20
cowboy-qc saddle   ts_guess.pdb                          # multi-backend saddle search
cowboy-qc irc      ts.pdb --step 0.1                     # IRC from a TS
cowboy-qc neb      reactant.pdb product.pdb              # NEB + CI-NEB
cowboy-qc ts       input.pdb --strategy irc              # legacy TS pipeline
```
...
The installable package is **`quantum_engine`**; the CLI binary stays **`cowboy-qc`**.
...
| **`quantum_engine.prep`** | Active-site extraction, protonation (`protonator` = `cowboy-qc protonate`), charge calculation |
...
The package is **not yet pip-installed in the container**, so the `cowboy-qc` console
...
> Once the package is `pip install`-ed into a rebuilt container, `cowboy-qc <op>`
...
quantum_cowboy_biochemistry/
    quantum_engine/             # the installable Python package (CLI: cowboy-qc)
        cli.py                  # `cowboy-qc <op>` entry point
        ...
        prep/                   # protonator (cowboy-qc protonate), extraction, charge
